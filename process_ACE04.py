...
def get_filename(file_list_path):
    file_name_list = []
    with open(file_list_path, "r", encoding="utf-8") as f:
        content = f.read().splitlines()    
    for c in content[1:]:
        if c.split('\t')[2]=='newswire' or c.split('\t')[2]=='broadcast_news':
            file_name_list.append((c.split('\t')[0], c.split('\t')[2]))
    
    return file_name_list



def get_apf_and_sgm(data_path):
    apf_file = []
    sgm_file = []
    ...
    for root, dirs, files in os.walk(data_path):
        for f in files:
            ...
            if fullpath.endswith('apf.xml'):
                apf_file.append(fullpath)

            if fullpath.endswith('.sgm'):
                sgm_file.append(fullpath)
                
    return apf_file, sgm_file


def sort_apf_and_sgm(file_name_list, apf_file, sgm_file):
    apf_file_sort = []
    sgm_file_sort = []

    for file in file_name_list:
        for apf, sgm in zip(apf_file, sgm_file):
            if file[0]==apf.split('/')[-1].replace('.apf.xml', ''):
                apf_file_sort.append(apf)
            if file[0]==sgm.split('/')[-1].replace('.sgm', ''):
...
def process_apf(apf_file):
    ...
    for relation in apf_root.iter('relation'):
        ...
        if rel_type == "DISC":
            sys.stderr.write("ignore DISC\n")
            continue
        for mention in relation.iter('relation_mention'):
            rel_id = mention.attrib["ID"]
            rel = [rel_id, rel_type, "", ""]
            ignore = False
            for arg in mention.iter('rel_mention_arg'):
                arg_id = arg.attrib["ENTITYMENTIONID"]
                if arg_id in ne_map:
                    arg_id = ne_map[arg_id]
                rel[int(arg.attrib["ARGNUM"])+1] = arg_id
...
def process_char2word(doc, ent_seqs, rel_seqs):
    ...
    return sentence_list, ent_lists, rel_lists
...
def check_doc_type(file_name_list, apf_file_name, combine_input):
    subdirectories = ''
    for each_tuple in file_name_list:
        if apf_file_name in each_tuple:
            subdirectories  = each_tuple[1]
            break
            
    if subdirectories == 'broadcast_news':
        return combine_input[3:-1]
    elif subdirectories == 'newswire':
        if apf_file_name[:3] == 'APW':
            return combine_input[5:]
        elif apf_file_name[:3] == 'NYT':
            return combine_input[7:-1]


ACE04_root = 'data/ACE04'
process_path = os.path.join(ACE04_root, 'process_data')
os.makedirs(process_path, exist_ok=True)    

file_list_path = os.path.join(ACE04_root, 'data/English/FileList_English')
file_name_list = get_filename(file_list_path)  

data_path = os.path.join(ACE04_root, 'data/English')
apf_file, sgm_file = get_apf_and_sgm(data_path)

apf_files, sgm_files = sort_apf_and_sgm(file_name_list, apf_file, sgm_file)
...
for apf_file, doc in zip(apf_files, sgm_files):
    ...
    combine_input = check_doc_type(file_name_list, apf_file_name, combine_input)
